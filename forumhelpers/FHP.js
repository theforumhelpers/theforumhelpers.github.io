      ...
      for (j = 0; j < data.length; j++) {
        ...
        if (j == data.length - 1 && dataSet != "curators") {
          ...
          document.getElementById("totalNumber").innerHTML =
            parseInt(document.getElementById("managersNumber").innerHTML) +
            parseInt(document.getElementById("curatorsNumber").innerHTML);
        }
...
function getCount(name) {
  fetch("https://scratchdb.lefty.one/v3/forum/user/info/" + name)
    ...
    .then((data) => {
      ...
      document.getElementById(name + "Posts").innerText =
        " - " + posts + " Posts";
    });
}
